...
void loop() {
  ...
  if (BTserial.available() > 0) {
    ledBrightnessLevel = (int)BTserial.read();
    int ledOutputLevel = ledBrightnessLevel * 25 / 10 + 125;
    Serial.print("Read from bt: ");
    Serial.println(String(ledBrightnessLevel));
    Serial.print("Output to pin: ");
    Serial.println(String(ledOutputLevel));
    analogWrite(ledOutputPin, ledOutputLevel);
  }
...
//  Serial.println("Reading from solar panel");
  solarPanelVoltage = analogRead(solarInputPin);
  solarPanelVoltage = ((float)solarPanelVoltage * 5.0F) / 1024.0F;
//  Serial.println(String(distance) + "/" + String(dhtTemperature) + "/" + String(d18b20Temperature) + "/" + String(solarPanelVoltage) + "#");
  BTserial.print(String(distance) + "/" + String(dhtTemperature) + "/" + String(d18b20Temperature) + "/" + String(solarPanelVoltage) + "#");
